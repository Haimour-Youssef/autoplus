...
var TableData = function () {
	...
	var runEditableTable = function(){
		...
		function editRow(oTable, nRow) {
			var aData = oTable.fnGetData(nRow);
			var jqTds = $('>td', nRow);
			jqTds[0].innerHTML = '<input id="nom-new" name="nom" type="text" class="form-control" value="' + aData[0] + '"><div class="text-danger nom-error"></div>';
			jqTds[1].innerHTML = '<input name="gsm" type="text"  value="' + aData[1] + '">';
			jqTds[2].innerHTML = '<input name="adresse" type="text"  value="' + aData[2] + '">';
			jqTds[3].innerHTML = '<input type="text" class=" no-display" value="' + aData[3] + '">';
			
			
			
			jqTds[4].innerHTML = '<a class="save-row" href="">Save</a>';
			jqTds[5].innerHTML = '<a class="cancel-row" href="">Cancel</a>';
		...
		function saveRow(oTable, nRow) {
			...
			oTable.fnUpdate('<a class="edit-row" href="">Edit</a>', nRow, 4, false);

			if (jqInputs[3].value == 0)
			oTable.fnUpdate('<a class="delete-row" href="">Delete</a>', nRow, 5, false);
			else oTable.fnUpdate('<a></a>', nRow, 5, false);
		...
		$('#sample_2').on('click', '.save-row', function(e) {
			...
			$.ajax({
				...
				error : function (data) {
					...
					// nom
					$('#nom-new').addClass('has-error');
					$('.nom-error').text(data.responseJSON.errors.nom);
