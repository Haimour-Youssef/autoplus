...
function errorsValidationAjax(reject){


    $('.text-danger').remove();$('.has-error').removeClass('has-error');

    var response = $.parseJSON(reject.responseText);
    $.each(response.errors, function (key, val) {
        $element = $('.form-control[name="' + key + '"]');

        $element.addClass('has-error');
        $element.after( "<span class='text-danger'>" + val[0] + "</span>" );
    });
        ...
        function remplir_pie($placeholder,$tr,$libelle,$count)
        {
        var data_pie = [];

        jQuery('.' + $tr).each(function (index) {
             data_pie[index] = {
                label: $(this).find('.' + $libelle).text(),
                data: Math.floor($(this).find('.' + $count).text() * 100) + 1
            };

        });


            $.plot('#' + $placeholder, data_pie, {
            series: {
                pie: {
                    show: true,
                    radius: 1,
...
$('.btn-confirmation').on( "click", function() {
    ...
    $('#btn-submit').attr('data-form-submit',$( this ).attr('data-form-submit'));
    
    $('#static .modal-body p').text($( this ).attr('data-body-text'));
    $('#btn-submit').text($( this ).attr('data-button-text'));
    $('#btn-submit').attr('class','btn btn-' + $( this ).attr('data-button-type'));
...
$('.link_cli').on( "click", function(e) {

    e.preventDefault();
    $("#" + $( this ).attr('data-href')).trigger('click'); 
...
$('.viderBtn').on( "click", function(e) {
    e.preventDefault();
    $('.date-range').val('');
    $('.form-search').submit();
});
//id="btn-submit" type="button" data-form-submit=""
...
$('a[data-object="' + $('.main-content').attr('data-object') + '"]').parents('li').addClass('open active');
